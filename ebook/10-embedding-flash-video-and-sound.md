...
## 🏠 HOMEWORK & SUBMISSION — Session 10

After class, this session has exactly one deliverable set: **Homework 10**.
Everything you need sits on one page — the task list, the video brief and the
reference rubric: [homework/session-10/homework.md](../homework/session-10/homework.md).
...
2. **Explain — Part 2 (video, 4 points).** Record a **60–120 second** OBS
   screen recording presenting ONE part of this homework — for example: show the media page and explain how the `<video>` (or `<audio>`) element with `controls` works, including the fallback text and the `poster`/`source` attributes you used.
