/* INS2053 canvas QA sweep — Node port of qa-canvases.sh.
   This machine has no working bash (WSL /bin/bash missing, no Git Bash), so the
   shell version cannot run here. Same 9 checks, same exit-code contract:
   exit 1 if any hard check fails. SMALL-TYPE and UNUSED-IMPORT stay advisory.
...
console.log("== 8. non-ASCII characters inside svg (labels must be English) =====");
try {
...
console.log("== 9. estimated text width vs viewBox (labels must not run off) ====");
try {
