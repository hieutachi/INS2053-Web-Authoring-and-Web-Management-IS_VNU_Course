/* Fails if text inside an <svg> block contains a character that is not English.
   INS2053 is taught to international students, so every diagram label must be
   ...
   Usage: node check-svg-ascii.js <dirContainingCanvases> */
const fs = require("fs");
...
for (const f of fs.readdirSync(dir).filter((n) => n.endsWith(".canvas.tsx"))) {
