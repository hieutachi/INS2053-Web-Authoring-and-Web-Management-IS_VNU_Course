...
console.log("== 1. page inventory ==============================================");
{
  ...
  // index.html + orientation.html + ai-agents.html + cham-bai.html. The count is asserted rather
  // than relaxed, so an unexpected file appearing at the root is a failure.
  if (groups.root !== 4) bad(`expected 4 root pages (index + orientation + ai-agents + self-check tool), found ${groups.root}`);
