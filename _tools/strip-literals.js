/* Blank out comments and string/template literals in a TSX source so bracket
   counting only sees code. Used by check 1 of the QA sweep, which used to report
   false BAD lines for files whose prose contained "1)" or "{".
   ...
   Single quotes are the hard case, because this package uses them BOTH ways:

     row(24, 'alt="image"', ...)          <- a real JS string  (buoi-03:154)
   ...
   or end of line). Anything else is treated as an ordinary character. Every
   single-quoted string in canvases/ today matches the first shape and no
   apostrophe in prose does; see _tools/test-strip-literals.js.
